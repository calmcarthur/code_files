...
DENSITY = 998
MU = 0.001  # Pa.s
DIAMETER_TUBE = 0.00794  # m
AREA_TUBE = (np.pi * ((DIAMETER_TUBE/2)**2))  # m^2
AREA_BUCKET = 0.32 * 0.26 # m^2
...
def drainTime(l, tjoint):
    ...
    while h >= HEIGHT_BOTTOM:
        ...
        while tol > TOLERANCE:
            ...
            v2_new, re = calculateV2(v2, h, l, tjoint)
        ...
        # to not include the last
        if (h - ((DELTA_TIME * v1)) < 0):
            break
        else:
            h -= DELTA_TIME * v1
        
        res.append(re)
        # print("Height: ", h, ", Re: ", re, ", L or T: ", "Laminar" if re <= 2300 else "Turbulent", ", V2: ", v2, sep='')
...
def main():
    ...
    for l, tjoint in zip(TUBE_LENGTHS, T_JOINTS):
        time, res = drainTime(l, tjoint)
        print("Minimum Re: ", min(res))
