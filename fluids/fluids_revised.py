...
DIAMETER_TJOINT = 11.25 / 1000 # m
AREA_TJOINT = (np.pi * ((DIAMETER_TJOINT/2)**2))  # m^2
...
# Calculation of v2.
# K and outlet_area change depending on if there is a t-joint or not.
def calculateV2(v2, h, l, tjoint):

    re = calculateRe(v2)
    f = calculateF(re)
    outlet_area = AREA_TUBE if tjoint == 0 else AREA_TJOINT

    # Keep l because overall height doesn't change significantly with t-joint.
    z1 = h + 0.02 + (l/150)

    # Calculation.
    numerator = 2 * GRAVITY * z1
    term1 = 1
    term2 = -((outlet_area/AREA_BUCKET)**2)
    term3 = (l/DIAMETER_TUBE)*f
